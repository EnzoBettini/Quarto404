<template>
  <div class="menu-container">
    ...
    <div class="overlay-content" :class="{ 'video-playing': isVideoPlaying }">
      <h1 class="game-title">QUARTO 404</h1>

      <div v-if="!isVideoPlaying" class="menu-buttons">
        <button @click="startGame" class="game-button start-button">Começar Jogo</button>
        <button class="game-button" disabled>Opções</button>
        <button class="game-button" disabled>Sair</button>
      </div>
    </div>
  </div>
</template>
...
// --- NOVA LINHA: Importar o arquivo de áudio ---
// Atenção ao caminho exato e use barras normais '/'
...
const startGame = () => {
  // --- NOVA SEÇÃO: Tocar o som de início ---
  try {
    const audio = new Audio(startSoundFile);
    audio.play().catch(error => {
      // O catch aqui é para o caso de o navegador bloquear o play, embora por um clique geralmente funcione.
      console.error("Erro ao tentar tocar o som de início:", error);
    });
  } catch (error) {
    console.error("Erro ao criar o objeto de áudio:", error);
    // Isso pode acontecer se o caminho do áudio estiver incorreto na importação e 'startSoundFile' for undefined.
  }
  // --- FIM DA NOVA SEÇÃO ---
...
<style scoped>
/* Seus estilos permanecem os mesmos daqui para baixo... */
.menu-container {
  position: relative;
  width: 100vw;
  height: 100vh;
